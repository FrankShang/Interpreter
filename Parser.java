...
{	
	public static HashMap<String, FunData> funMap = new HashMap<String, FunData>();  //funData //funMap
	...
	
	{
		...
		if (state == State.Eq )
		{
			getToken();
			Exp exp = exp();
			FunData funData = funMap.get(header.funName); //funData //funMap //funName is the key value is the FunData
			funData.bodyExp = exp;						//funData
	...
	
	{
		if ( state.isType() )
		{
			...
			if ( state == State.Id )
			{
				...
				FunData funData = new FunData();                //funData	
				funData.parameterList = parameterList;			//funData
				funMap.put(funName, funData);					//funData //funMap
